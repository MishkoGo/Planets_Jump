...
public class DrawAnalogJoystickUI : MonoBehaviour,ICanSwitch
{
    #region VAR
    ...
    [SerializeField] private float _maxDist = 70;
    ...
    private void JoyControl()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (Vector3.Distance(beginPos, curPos) > _minDistance)
            {
                ...
                if (_direction.magnitude > _maxDist)
                {
                    _direction = _direction.normalized * _maxDist;
                }
            }
            else
            {
